...
//	alt + shift + s -> override/implement methods 부모클래스 자동완성기능
	@Override  // 기능이 있는 주석 (문법 체커기)틀리면 바로 체크해준다.
	void characterInfo() {
		super.characterInfo();
	...
	/*
	 ...
	 - 부모가 물려주는 메서드는 모든 자식드렝게 다 맞게 설계되기 힘들기 때문에
	 ...
	 -------------------------------------------------------------------
	 - 오버라이딩의 규칙 3가지 (3가지 모두 일치해야 오버라이딩 인정)

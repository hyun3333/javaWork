	...
	public static void main(String[] args) {
        ...
        */
		...
		while (true) {
			...
			if(2>full || 4<full) { // 참가자의 인원이 잘못 됐을때
				System.out.println("잘못 입력하셨습니다.");
				continue;
			} else if (2<=full || full<=4) { // 참가자의 인원
		...
		String[] userNames = new String [full]; // 참가자 인원의 배열
		
		String user = "";
		
		for (i=0; i<full; i++) { // 참가자 인원의 배열에 이름을 저장하는 위치
		...
		while (true) {
			...
			System.out.print("> ");
			bullet = sc.nextInt(); // 총알의 개수가 맞지 않을때
			if (bullet<1 || bullet>5) {
				System.out.println("총알의 개수가 맞지 않습니다");
				continue;
			} else if (bullet>0 || bullet<6) { // 총알의 개수
		...
		boolean[] bulletPos = new boolean[6]; // 실린더의 배열
...
//		System.out.println(human); //난수 발생하는지 확인
		...
		while (true) {
			...
			else { // 총을 쐈을때 실탄이 아닐경우 다음 배열로 넘기는 과정
				System.out.println(userNames[human] + "님이 생존하셨습니다.");
				human++;
			}
			if (human == userNames.length) { // 마지막 배열에서 첫번째 배열인 [0]으로 되돌리는 과정
				human = 0;
			}
			if (count == 5) { // 총알배열이 다 떨어졌을때 브레이크를 걸어 종료
				System.out.println("총알이 다 떨어져 " + userNames.length + " 명이 생존 하였습니다.");
				break;
			} 
			else if (userNames.length == 0) {
				System.out.println("모두 사망하여 게임이 종료되었습니다.");
				break;
			}
			if (bullet == 0) { // 실탄배열이 다 떨어졌을때 브레이크를 걸어 종료
				System.out.println("총알을 모두 사용하여 " + userNames.length + " 명이 생존 하였습니다.");
				break;
			}
			count++;
		}

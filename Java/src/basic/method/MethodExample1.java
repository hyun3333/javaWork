...
public class MethodExample1 {
	/*
		 ...
		 - 메서드의 선언은 메서드 내부에서는 불가능합니다. 새로운 메서드의 선언은 반드시
		   메서드 블록 외부 and 클래스 블록 내부에서만 가능합니다.
	...
	// 메서드의 선언 (static은 뒤에서 알게되니 지금은 없다고 생각한다.)
	static int calcTotal(int x) {
		int total = 0;
		for(int i=1; i<=x; i++) {
			total += i;
		}
		return total; // return값은 없으면 안된다
	}

	public static void main(String[] args) {
		 
		int total = calcTotal(10); // 함수의 호출문 
		// int total = 55

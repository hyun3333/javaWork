...
public class MethodExample2 {
	...
	// 1. x부터 y까지 누적합계를 구하는 메서드
	
	static int calcRangeTotal(int x, int y) { // 255개까지 선언이 가능하다. 
//		                                         4개 이상이 넘어가면 힘들어진다.
		int total = 0;
		for(int i=x; i<=y; i++) {
			total += i;
...
//	  매개변수가 필요없는 경우 괄호를 지울경우 변수로 선언한다 
//	  메서드로 선언할땐 괄호는 필수다.
...
//	3. 정수 n개를 전달하면 그 정수들의 총 합을 리턴하는 메서드
...
//		
//		System.out.println(calcRangeTotal(3, 8));
//		
//		2.

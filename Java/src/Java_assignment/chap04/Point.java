package Java_assignment.chap04;
...
public class Point {
	...
	public void showAllPoints(List<Student> sList) { 
		//학생 객체들이 들어있는 리스트가 와야죠?
		showPointUI();
	...
	boolean flag = false;
	public void searchPoint(List<Student> student) {
		...
		if (!flag){
			System.out.println("학번이 없습니다 메뉴로 돌아갑니다.");
		}
		/*
			...
			if(stu.getStuId().equals(stuNum)) {
				flag = true;
					...
					try {
						...
						System.out.println("당신의 점수를 제대로 입력해주세요");
						sc.nextLine();
						continue;
					}
	...
	public void deletePoint(List<Student> students) {
		...
		for(Student stu : students) {
			if(stu.getStuId().equals(stuNum)) {
				...
				String del = sc.next();
				if (del.equals("y") || del.equals("Y") || del.equals("ㅛ")) {
					students.remove(stu);
					System.out.println(stu.getName() + "님의 정보가 삭제되었습니다.");
					break;
				} else if (del.equals("n") || del.equals("N") || del.equals("ㅜ")) {
